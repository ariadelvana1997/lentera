...
import { 
  Rocket, Plus, Search, Loader2, Edit3, 
  Trash2, CheckCircle2, XCircle, Sparkles
} from "lucide-react"
...
export default function TemaKokurikulerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && selectedTheme) {
        await supabase.from('tema_kokurikuler').update(formData).eq('id', selectedTheme.id)
      } else {
        await supabase.from('tema_kokurikuler').insert([formData])
      }
      setIsDialogOpen(false)
      fetchData()
      alert("✅ Tema Kokurikuler Berhasil Disimpan!")
    } catch (err: any) { alert(err.message) }
    finally { setLoading(false) }
  }

  const handleDelete = async (id: string) => {
    if (confirm("Hapus tema kokurikuler ini?")) {
      await supabase.from('tema_kokurikuler').delete().eq('id', id)
      fetchData()
    }
  }
  ...
  const handleOpenAdd = () => {
    setIsEditMode(false)
    setFormData({ tema: "", is_active: true })
    setIsDialogOpen(true)
  }
        ...
        <Table>
          <TableHeader className="bg-muted/10">
            <TableRow className="border-none">
              ...
              <TableHead className="font-black text-[10px] uppercase text-primary">Tema Kegiatan Kokurikuler</TableHead>
          ...
          <TableBody>
            {fetching ? (
              ...
              <TableRow><TableCell colSpan={4} className="h-40 text-center text-muted-foreground font-medium italic">Belum ada data tema kokurikuler.</TableCell></TableRow>
            ) : themes.map((item, index) => (
              <TableRow key={item.id} className="hover:bg-muted/5 transition-colors border-border/50">
                <TableCell className="text-center font-bold text-muted-foreground p-6">{index + 1}</TableCell>
                <TableCell className="font-bold text-sm uppercase leading-relaxed tracking-tight max-w-md">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px] rounded-[2.5rem] p-8 border-none shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            <DialogHeader>
              <DialogTitle className="text-2xl font-black">{isEditMode ? "Edit Tema" : "Tambah Tema Projek"}</DialogTitle>
              <DialogDescription>Input tema kokurikuler (P5) sesuai panduan Kemendikbudristek.</DialogDescription>
            </DialogHeader>
            
            <div className="space-y-4">
              <div className="grid gap-2">
                <Label className="text-[10px] uppercase font-black ml-1">Nama Tema Projek</Label>
                <Input 
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl bg-muted/20">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[9px] text-muted-foreground italic">Tema non-aktif tidak muncul di pemilihan projek.</p>
                </div>
                <Switch 
            ...
            <DialogFooter>
              <Button type="submit" className="w-full h-12 rounded-xl font-black shadow-lg shadow-primary/20" disabled={loading}>
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "SIMPAN TEMA PROJEK"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
