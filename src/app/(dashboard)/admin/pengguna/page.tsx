...
// Pastikan path ini sesuai dengan tempat kamu menyimpan server action tadi
import { registerUserAction } from "@/app/actions/register-user" 
import { 
  Search, UserPlus, Edit2, Trash2, Loader2, Filter
} from "lucide-react"
...
export default function PenggunaPage() {
  ...
  // Fungsi Toggle Role yang Aman (Mencegah Infinite Loop)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode) {
        ...
        alert("Pengguna baru berhasil terdaftar dan SIAP LOGIN!")
      }
                ...
                <Table>
                  <TableHeader className="bg-muted/20">
                    <TableRow className="hover:bg-transparent border-none">
                      <TableHead className="font-black text-[10px] uppercase tracking-widest p-6">Pengguna</TableHead>
                      <TableHead className="font-black text-[10px] uppercase tracking-widest">Hak Akses</TableHead>
                      <TableHead className="font-black text-[10px] uppercase tracking-widest">Dibuat</TableHead>
                      <TableHead className="text-right font-black text-[10px] uppercase tracking-widest p-6">Aksi</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {fetching ? (
                      ...
                      <TableRow><TableCell colSpan={4} className="h-40 text-center text-muted-foreground font-medium">Tidak ada data pengguna ditemukan.</TableCell></TableRow>
                    ) : (
                      getFilteredUsers(tabValue).map((user) => (
                        <TableRow key={user.id} className="hover:bg-muted/10 transition-colors border-border/50">
                          ...
                          <TableCell className="text-right p-6">
                            <div className="flex justify-end gap-1">
                              <Button onClick={() => handleEdit(user)} variant="ghost" size="icon" className="rounded-xl hover:bg-primary/10 hover:text-primary transition-colors">
                                <Edit2 className="w-4 h-4" />
                              </Button>
                              <Button onClick={() => handleDelete(user.id)} variant="ghost" size="icon" className="rounded-xl hover:bg-red-50 hover:text-red-500 transition-colors">
                  ...
                  {ROLE_OPTIONS.map((role) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span className="text-[11px] font-bold leading-none cursor-pointer flex-1 py-1">
                          {role.label}
                        </span>
                      </label>
                    )
